...
<head>
  ...
  <style>
    ...
    color: #000;
    padding: 4px 6px;
    margin-top: 15px;/* Adicione margem à esquerda para separar do campo de busca */
    background-color: #ddd;
    border-radius: 5px;
    font-size: 20px;
}
...
.btnbuscar:hover {
    background-color: rgb(177, 177, 177);
        ...
        .btnpacientes {
            margin: 5px 5px; /* Adicione margens superior e inferior de 5px e remova a margem esquerda e direita */
    font-size: 20px;
    font-family: Arial, sans-serif;
          text-decoration: none;
          color: white;
...
if (isset($_POST['busca'])) {
    ...
    $query = "SELECT * FROM pacientes WHERE nome LIKE '%$termo_busca%' ORDER BY nome";
        ...
        <table>
            ...
            <tbody>
                <?php while ($row = mysqli_fetch_assoc($result)) { ?>
                    <tr>
                        <td><a class="btnpacientes" href="-novaFicha.php?id=<?php echo $row['id']; ?>"><?php echo $row['nome']; ?></a></td>
                    </tr>
                <?php } ?>
            </tbody>
        </table>
